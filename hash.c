...
{
    log_info("creating a new hash table element");
...
{
    log_info("Deleting an hash table element");
...
{
    log_info("Creating a new hash table");
    hash_table_t *table = calloc(1, hash_table_s);
    if (!table) 
    {
        log_error("No Memory while allocating hash_table");
        return NULL;
    }
    table->mode = mode;
    table->key_num = 128;
    ...
    {
        log_error("No Memory while allocating hash_table store house");
        free(table);
        return NULL;
    }
    return table;
...
{
    log_info("Deleating a hash table");
    size_t i=0;
    for (;i<HASH_LEN;i++)
    {
...
int hash_table_add(hash_table_t * table, void * key, size_t key_len, void * value, size_t value_len)
{
    ...
    if (!element)
    {
        log_error("Cannot allocate memory for element");
        return -1; // No Memory
    }
    if (table->mode == MODE_COPY)
    {
        log_info("Adding a key-value pair to the hash table with hash -> %d, in COPY MODE", (int)hash);
    ...
    {
        log_info("Adding a key-value pair to the hash table with hash -> %d, in VALUEREF MODE", (int)hash);
        element->key = malloc(key_len);
        if (element->key)
        {
            memcpy(element->key, key, key_len);
    ...
    {
        log_info("Adding a key-value pair to the hash table with hash -> %d, in ALLREF MODE", (int)hash);
        element->key = key;
        element->value = value;
    }
    ...
    {
        log_info("No Conflicts adding the first element at %d", (int)hash);
        table->store_house[hash] = element;
        table->key_count++;
    }
    else
    {
        log_info("Conflicts adding element at %d", (int)hash);
        ...
        while(temp->next)
        {
            ...
            if(temp->next)
            {
                ...
                {
                    log_info("Found Key at hash -> %d", (int)hash);
...
{
    log_info("Deleting a key-value pair from the hash table");
    ...
    if (!table->store_house[hash])
    {
        log_error("Key Not Found -> No element at %d", (int)hash);
    ...
    while(temp)
    {
        ...
        if(temp)
        {
            if (!memcmp(temp->key, key, key_len))
            {
                ...
                hash_table_element_delete(table, temp);
                log_info("Deleted a key-value pair from the hash table");
                table->key_count--;                
                return 0;
            }
            prev=temp;
...
{
    size_t hash = HASH(key, key_len);
    log_info("Looking up a key-value pair for hash -> %d", (int)hash);
    if (!table->store_house[hash])
    {
        log_error("Key not found at hash %d, no entries", (int)hash);
    ...
    log_info("Key not found at hash %d", (int)hash);
    return NULL; // key not found   
}
...
{
    size_t hash = HASH(key, key_len);
    log_info("Searching for key with hash -> %d", (int)hash);
    if (!table->store_house[hash])
    {
        log_error("Key not found with hash -> %d, no entries", (int)hash);
    ...
    while(temp)
    {
        ...
        if(temp)
        {
            if (!memcmp(temp->key, key, key_len))
            {
                log_info("Key Found with hash -> %d", (int)hash);
                return 1; // key found
            }
            temp=temp->next;
        }
    }
    log_info("Key not found with hash -> %d", (int)hash);
    return 0; // key not found   
}
...
size_t hash_table_get_keys(hash_table_t * table, void ** keys)
{
    ...
    for(i=0;i<HASH_LEN;i++)
    {
        if (table->store_house[i])
        {
            ...
            #ifdef DEBUG
            log_debug("found %d key(s) at hash -> %d", (int)num, (int)i);
            #endif 
        }
    }
    return count;
...
size_t hash_table_get_elements(hash_table_t * table, hash_table_element_t *** elements)
{
    ...
    if (!*elements) 
    {
        log_error("No Memory to allocate elements array");
        return 1;
    }
    for(i=0;i<HASH_LEN;i++)
    {
...
{
    log_info("resizing hash table from %d to %lu", table->key_num, len);
    ...
    if (!count) 
    {
        log_error("Got No Elements from the hash table");
        return -1;
    }
    ...
    if (!table->store_house)
    {
        ...
        log_error("No Memory for new store house");
        return -2;
    }
    table->key_num = len;
...
int hash_add(hash_table_t *table, const char *key, void *data)
{
    return hash_table_add(table, key, strlen(key) + 1, data, sizeof(data));
}

int hash_remove(hash_table_t *table, const char *key)
{
    return hash_table_remove(table, key, strlen(key) + 1);
}

void *hash_lookup(hash_table_t *table, const char *key)
{
    return hash_table_lookup(table, key, strlen(key) + 1);
}
